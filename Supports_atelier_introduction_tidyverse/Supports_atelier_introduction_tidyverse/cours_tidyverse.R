...
library(readxl)
exemple1 <- read_excel("./Supports_atelier_introduction_tidyverse/Supports_atelier_introduction_tidyverse/exempleexcel.xlsx", na = "noobserv", col_names=TRUE)
head(exemple1)
...
dataairquality %>% group_by(Mois) %>% summarise(moyWind=mean(Wind))
...
dataairquality %>% group_by(Mois) %>% summarise(n=n())
